...
app.post(
  ...
  async (req, res) => {
    ...
    var token = req.cookies.login_tokeny
    ...
    con.query(
      `insert into employee_basic_infomation (reg_id,firstname,lastname,birth_date,address,gender,phone_number,relationship,state,city) values('${login_user__id}','${firstname}','${lastname}','${birth_date}','${address}','${gender}','${phone_number}','${relationship}','${state}','${city}') ;`,
      function (error, data) {
        if (error) throw error;
        id = data.insertId;
        console.log(id);
