  ...
  let search = req.query.search || "";
    ...
    for (var s = 0; s < total.length; s++) {
      ...
      if (on.includes(reg)) {
        ...
        var empl = await alldata(
          `select profile_pic, employee_basic_infomation.reg_id,firstname,email,phone_number,designation,department,time_stamp from employee_basic_infomation INNER JOIN document_master ON employee_basic_infomation.reg_id = document_master.reg_id where employee_basic_infomation.reg_id = ${reg}  and employee_basic_infomation.firstname like "%${search}%"; `
        );
